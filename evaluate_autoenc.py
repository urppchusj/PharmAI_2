...
if __name__ == '__main__':
	...
	autoencoder = tf.keras.models.load_model(os.path.join(model_dir, 'trained_model.h5'), custom_objects={'Encoder':Encoder, 'Decoder':Decoder, 'FeatureExtractor':FeatureExtractor, 'encoder_loss':encoder_loss, 'autoencoder_accuracy':autoencoder_accuracy, 'autoencoder_false_neg_rate':autoencoder_false_neg_rate})
	...
	for idx, batch in enumerate(tqdm(test_ds)):
		...
		batch_x_hat = autoencoder.predict_on_batch(batch_x)
		x.append(batch_x) 
	...
	results = autoencoder.evaluate(x, x, verbose=1)
	...
	with open(os.path.join(model_dir,'evaluation_results.txt'), mode='w', encoding='cp1252') as file:
		...
		for metric, result in zip(autoencoder.metrics_names, results):
